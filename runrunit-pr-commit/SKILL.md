...
- **Projeto omie (Next.js + Strapi):**
  - **Branch/commit/PR/comentário — `omie-commit`** (skill **global**, em `~/.cursor/skills`): padrão de branch (`feat/<descricao>`), commit (Conventional Commits + corpo detalhado), **template de PR do omie** e **comentário no Runrun.it com hash do merge** (deploy manual pela Omie via John). Usar sempre que for branchar/commitar/abrir PR/comentar em task do repo omie.
